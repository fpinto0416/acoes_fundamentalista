...
Monta, dia a dia, uma base histórica de dados fundamentalistas (Yahoo
Finance) das ações do **IBrA** (Índice Brasil Amplo — carteira teórica
oficial da B3, cobre ~99% do valor de mercado negociado na bolsa). Por
enquanto o objetivo é só **coletar e acumular histórico**; a análise em
cima disso vem depois, quando já houver uma série de dias/semanas
armazenada.

> Repo separado desde 25/08/2026 — nasceu como subpasta de
> [`mia_telegram`](https://github.com/fpinto0416/mia_telegram) (commit
> `75ced7a`) e foi desmembrado pra cá por não ter nada a ver com o pipeline
> de sinais daquele repo (direção/volatilidade/opções). Código e dados
> idênticos, só mudou o endereço — nenhum caminho interno (scripts,
> imports) precisou mudar, só o workflow (removido o prefixo
> `acoes_fundamentalista/` dos comandos, já que agora é a raiz do repo).
...
```bash
FORCE_UPDATE=1 python scripts/update_tickers_ibra.py
```
...
`.github/workflows/coleta_diaria.yml` roda todo dia útil às 19:00 (horário
de Brasília), depois do fechamento da B3: coleta os fundamentos
(`download_fundamentals.py`), gera o painel (`gerar_panorama.py`) e faz
commit + push de `dados/` e `tickers/` de volta pro repositório. Também
...
python scripts/gerar_panorama.py
```
